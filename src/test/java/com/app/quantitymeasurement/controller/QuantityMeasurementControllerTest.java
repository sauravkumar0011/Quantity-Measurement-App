 ...
 *
 * Responsibilities verified: - Each perform* method correctly delegates to the
 * service layer - Controller returns the service result without modification -
 * Controller accepts a service via constructor (dependency injection) -
 * Controller contains NO business logic of its own - All 5 operations (compare,
 * convert, add, subtract, divide) are routed correctly - Layer separation:
 * controller is independently testable via a stub service
 *
...
public class QuantityMeasurementControllerTest {
	 ...
	 *
	 * A stub service is injected — controller delegates and returns the stub's
	...
	public void testLayerSeparation_ControllerIndependence_StubService() {
		...
		IQuantityMeasurementService stub = new IQuantityMeasurementService() {
			...
			public QuantityDTO convert(QuantityDTO a, QuantityDTO b) {
				return a;
			}
			...
			public QuantityDTO add(QuantityDTO a, QuantityDTO b, QuantityDTO t) {
				return a;
			}
			...
			public QuantityDTO subtract(QuantityDTO a, QuantityDTO b, QuantityDTO t) {
				return a;
			}
		...
		QuantityMeasurementController stubController = new QuantityMeasurementController(stub);
	 ...
	 *
	 * A null service reference causes NullPointerException when any operation is
	 * attempted — the controller has no null guard of its own, which is the
	 * expected design (caller is responsible for providing a valid service).
	 */
	@Test
	public void testController_NullService_OperationThrows() {
		QuantityMeasurementController nullServiceController = new QuantityMeasurementController(null);
	...
	public void testPerformConversion_FeetToInches_CorrectResult() {
		QuantityDTO result = controller.performConversion(twoFeet, new QuantityDTO(0, QuantityDTO.LengthUnit.INCHES));
	...
	public void testPerformConversion_Temperature_CelsiusToFahrenheit() {
		QuantityDTO result = controller.performConversion(new QuantityDTO(100, QuantityDTO.TemperatureUnit.CELSIUS),
	...
	public void testPerformAddition_Weight_KilogramPlusGram() {
		QuantityDTO result = controller.performAddition(new QuantityDTO(1, QuantityDTO.WeightUnit.KILOGRAM),
	...
	public void testPerformAddition_Volume_LitrePlusMillilitre() {
		QuantityDTO result = controller.performAddition(new QuantityDTO(1, QuantityDTO.VolumeUnit.LITRE),
	...
	public void testPerformSubtraction_ThreeOperands_ExplicitTargetUnit() {
		QuantityDTO result = controller.performSubtraction(new QuantityDTO(10, QuantityDTO.LengthUnit.FEET),
				new QuantityDTO(6, QuantityDTO.LengthUnit.INCHES), new QuantityDTO(0, QuantityDTO.LengthUnit.FEET));
	...
	public void testPerformDivision_FourFeetOverTwoFeet_ReturnsTwo() {
		double result = controller.performDivision(new QuantityDTO(4, QuantityDTO.LengthUnit.FEET),
	...
	public void testDataFlow_InputPassedThrough_OutputReturnedUnmodified() {
		...
		QuantityDTO result = controller.performAddition(new QuantityDTO(1, QuantityDTO.LengthUnit.FEET),
